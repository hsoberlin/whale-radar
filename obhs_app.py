...
def scan_market(macro_data):
    ...
    for ticker_raw in raw_tickers:
        ...
        try:
            ...
            if len(h) < 2 or h.iloc[-1].isnull().any(): continue
            last = h.iloc[-1]
            prev = h.iloc[-2]
            ...
            results.append({
                "SYMBOL": ticker_raw, "CONF": score, "VOL_POWER": round(last['vol_power'], 2),
                ...
                "PORTO": "HIGH", "STRATEGY": strategy_tag, "RAW_DATA": h,
...
if data:
    ...
    col_m1.metric("INSTITUTIONAL TARGETS FOUND", len(df_display))
    col_m2.metric("LIQUIDITY CONFIRMATION", "PASSED")
    
    col_main, col_news = st.columns([3, 1])
    with col_main:
        st.dataframe(df_display[["SYMBOL","STRATEGY","VOL_POWER","PRICE","CHG%","VALUE"]], 
        ...
        for i in range(0, len(top_picks), 2):
            ...
            for idx, target in enumerate(top_picks[i:i+2]):
                with cols[idx]:
                    ...
                    st.markdown(f"<div class='thesis-box'><div class='thesis-header'>{target['GROUP']}</div>{target['THESIS']}</div>", unsafe_allow_html=True)
    with col_news:
